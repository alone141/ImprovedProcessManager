...
| [`manager/`](manager/) | `beraynetworkmanager`: a ZeroMQ identity router. Peers connect under a name and address each other by name; with `router_endpoint` configured, the manager serves commands through it as well, so several managers can share one router and clients reach them by identity. | C++20, CMake |
...
manager/build/src/beraynetworkmanager
manager/build/src/berayprocessmanager --restart sensor_fusion --router tcp://127.0.0.1:5558
